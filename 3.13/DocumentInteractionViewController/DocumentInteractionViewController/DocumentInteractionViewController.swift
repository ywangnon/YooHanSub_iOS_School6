...
final class DocumentInteractionViewController: UIViewController {
  // 델리게이트를 별도로 사용하지 않고 직접 구현
    // UIDocumentInteractionController: 파일 전송 컨트롤러
    //lazy - delegate = self 생성시점을 늦춰서 사용
  ...
  override func viewDidLoad() {
    ...
    // 밑의 preview 메소드에서 다른 파일에 관한 것을 테스트 해보려 생성
...
//    docInteractionVC.url = URL(fileURLWithPath: filePath)
  ...
  @IBAction private func sendFile(_ sender: Any) {
    ...
    // 데이터 콘테이너 파일을 다른 곳에 옮기고, 어떤 파일인지 알려준다.
    docInteractionVC.url = URL(fileURLWithPath: filePath)
